...
@router.post(
    ...
    summary="Créer un compte utilisateur (admin seulement)",
)
def create_account(
    payload: UserCreate,
...
def get_stats(
    ...
    presc_today = (
        ...
        .filter(Prescription.created_at >= today_start)
        .scalar()
        or 0
    )
    ...
    overrides = (
        db.query(func.count(AuditCdsHook.id))
        .filter(AuditCdsHook.decision == "OVERRIDE")
        .scalar()
        or 0
    )
    accepted = (
        db.query(func.count(AuditCdsHook.id))
        .filter(AuditCdsHook.decision == "ACCEPTED")
        .scalar()
        or 0
    )
    ...
    compliance_rate = (
        round((accepted / total_audit * 100), 1) if total_audit > 0 else 100
    )

    return {
        "total_users": len(users),
        "active_doctors": sum(
            1 for u in users if u.role.value == "doctor" and u.is_active
        ),
        "active_pharmacists": sum(
            1 for u in users if u.role.value == "pharmacist" and u.is_active
        ),
