...
class Prescription(Base):
    ...
    fhir_bundle_id = Column(UUID(as_uuid=True), unique=True, nullable=True)              # identifiant FHIR du bundle CDS Hooks
    status         = Column(String(20),  default="draft",      nullable=False)           # draft | alerts | safe
    hook_event     = Column(String(50),  default="order-sign", nullable=True)            # événement CDS Hooks déclencheur
    created_at     = Column(TIMESTAMP(timezone=True), server_default=func.now(), nullable=False)

    patient = relationship("Patient",          back_populates="prescriptions")
    doctor  = relationship("User",             foreign_keys=[doctor_id])
    lines   = relationship("PrescriptionLine", back_populates="prescription", cascade="all, delete-orphan")
...
class PrescriptionLine(Base):
    ...
    dci             = Column(String(255),    nullable=False, index=True)   # DCI telle que saisie par le médecin
    dose_mg         = Column(Numeric(12, 4), nullable=False)               # dose normalisée en milligrammes
    dose_unit_raw   = Column(String(20),     nullable=True)                # unité d'origine avant normalisation
    ...
    alerts       = relationship("CdsAlert",     back_populates="prescription_line", cascade="all, delete-orphan")
