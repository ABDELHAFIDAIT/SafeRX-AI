...
class _CapturingRouter:
    ...
    def post(self,   chemin, **kw): return self._capturer("POST",   chemin)
    def get(self,    chemin, **kw): return self._capturer("GET",    chemin)
    def patch(self,  chemin, **kw): return self._capturer("PATCH",  chemin)
    def delete(self, chemin, **kw): return self._capturer("DELETE", chemin)
...
def charger_handlers():
    ...
    if cle in sys.modules:
        del sys.modules[cle]
    import backend.app.api.endpoints.audit
...
class TestDecisionUnique(unittest.TestCase):
    ...
    def _appeler(self, payload, alerte):
        ...
        medecin = make_user(role="doctor"); medecin.id = 1
    ...
    def test_accepted_cree_une_entree_sans_validation(self):
        ...
        with patch("backend.app.api.endpoints.audit.validate_override_justification") as mock_val:
    ...
    def test_override_appelle_la_validation_semantique(self):
        ...
        with patch("backend.app.api.endpoints.audit.validate_override_justification",
                   return_value=resultat_llm) as mock_val:
    ...
    def test_alerte_introuvable_retourne_404(self):
        ...
        db      = make_db_session()
        db.query.return_value.filter.return_value.first.return_value = None  # ← introuvable
        medecin = make_user(role="doctor"); medecin.id = 1

        with self.assertRaises(HTTPException) as ctx:
            _creer_audit(payload=payload_simple(alert_id=9999), db=db, current_user=medecin)
    ...
    def test_snapshot_des_donnees_de_lalerte(self):
        ...
        """
        alerte = make_cds_alert(id=1, alert_type="RENAL", severity="MAJOR",
...
class TestDecisionsBulk(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.medecin = make_user(role="doctor"); self.medecin.id = 1
    ...
    def test_trois_decisions_creent_trois_entrees(self):
        ...
        with patch("backend.app.api.endpoints.audit.validate_override_justification",
            ...
            _creer_bulk(
                ...
                db=db, current_user=self.medecin,
            )
...
class TestAccesAdmin(unittest.TestCase):
    ...
    def test_medecin_obtient_403(self):
        ...
        from fastapi import HTTPException
        medecin = make_user(); medecin.role = Role.DOCTOR
    ...
    def test_pharmacien_obtient_403(self):
        ...
        from fastapi import HTTPException
        pharmacien = make_user(); pharmacien.role = Role.PHARMACIST
