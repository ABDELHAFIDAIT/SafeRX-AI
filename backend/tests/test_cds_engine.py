...
class TestAllergie(unittest.TestCase):
    ...
    def test_allergie_famille_penicilline(self):
        ...
        self.assertGreater(len(allergies), 0, "Allergie croisée Pénicilline→Amoxicilline attendue")
...
class TestRedondance(unittest.TestCase):
    ...
    def test_deux_fois_paracetamol(self):
        ...
        db = MagicMock()
        c1 = MagicMock(); c1.drug_id = 1; c1.dci = "paracétamol"; c1.position = 1
        c2 = MagicMock(); c2.drug_id = 2; c2.dci = "paracétamol"; c2.position = 1

        def simuler_query(model):
            from backend.app.models.drug import Drug, DciComponent
            from backend.app.models.drug_interaction import DrugInteraction
            q = MagicMock()
            if model is Drug:           q.filter.return_value.all.return_value = [drug1, drug2]
            elif model is DciComponent: q.filter.return_value.all.return_value = [c1, c2]
            else:                       q.filter.return_value.all.return_value = []
            return q
        db.query.side_effect = simuler_query
...
class TestAge(unittest.TestCase):
    ...
    def test_enfant_trop_jeune(self):
        ...
        alertes  = analyse_prescription(db=db, patient=patient, lines=[line])
        posologie = [a for a in alertes if a.alert_type == "POSOLOGY" and "Âge" in a.title]
...
class TestContraIndication(unittest.TestCase):
    ...
    def test_roaccutane_interdit_pendant_grossesse(self):
        ...
        drug    = make_drug(id=1, dci="isotrétinoïne",
    ...
    def test_lithium_interdit_pendant_allaitement(self):
        ...
        drug    = make_drug(id=1, dci="lithium",
                            contraindications="Éviter pendant l'allaitement.")
